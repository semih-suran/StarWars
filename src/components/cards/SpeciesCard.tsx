...
export const SpeciesCard = ({ data }: { data: Species }) => (
  <div className="bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:scale-105 transition-transform border border-gray-700">
    <SwapiImage category="species" id={getIdFromUrl(data.url)} name={data.name} />
    <div className="p-4">
      <h3 className="text-lg font-bold text-sw-yellow mb-2 truncate">{data.name}</h3>
      <div className="text-gray-400 text-xs space-y-1">
        <p>Class: <span className="text-gray-200">{data.classification}</span></p>
        <p>Language: <span className="text-gray-200">{data.language}</span></p>
      </div>
    </div>
  </div>
);